...
type SignOptions struct {
	*ParseOptions
	...
	// Issuer is the signer's verifiable GOBL Net address and Audience is the
	// optional GOBL Net audience the signature is bound to; either may be
	// empty.
	Issuer   net.Address
	Audience net.Address
}
...
func Sign(ctx context.Context, opts *SignOptions) (*gobl.Envelope, error) {
	...
	if opts.Issuer != "" {
		signOpts = append(signOpts, head.WithIssuer(opts.Issuer.String()))
	}
	if opts.Audience != "" {
		signOpts = append(signOpts, head.WithAudience(opts.Audience.String()))
	}
